		...
		<div className="space-y-6">
			...
			<div className="grid md:grid-cols-2 gap-4">
				{filteredSymptoms.map((symptom) => {
					const selected = symptoms.includes(symptom.label)
					return (
						<Card
							key={symptom.id}
							className={`cursor-pointer transition-colors border ${selected ? "border-blue-300 bg-blue-50" : "hover:bg-muted/50"}`}
						...
						>
							<CardContent className="p-4">
								<div className="flex items-start space-x-3">
									...
									{selected && <CheckCircle2 className="h-4 w-4 text-blue-600 mt-0.5" />}
								</div>
							</CardContent>
						</Card>
					)
