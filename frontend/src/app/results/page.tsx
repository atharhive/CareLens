...
import { ResultsPdfDoc } from "@/services/react-pdf"
import { 
	ArrowLeft, 
	Share2, 
	MapPin, 
...
export default function ResultsPage() {
	...
	const handleExportReactPdf = async () => {
		try {
			const { pdf: createPdf } = await import("@react-pdf/renderer")
			const doc = (
				<ResultsPdfDoc
					meta={{ age: demographics.age, sex: demographics.sex, date: new Date().toLocaleDateString() }}
					summary={{
						conditionsAnalyzed: conditions.length,
						avgRiskPct: Math.round(averageRiskScore * 100),
						highRiskCount: highRiskConditions.length,
						actionCount: recommendations ? (recommendations.immediate?.length || 0) : 0,
					}}
					risks={conditions.map(([k, v]) => ({
						name: k.replace(/([A-Z])/g, ' $1').replace(/^./, (s) => s.toUpperCase()),
						scorePct: Math.round(v.score * 100),
						category: v.category,
					}))}
					highRisk={highRiskConditions.map(([k, v]) => ({ name: k.replace(/([A-Z])/g, ' $1').replace(/^./, (s) => s.toUpperCase()), scorePct: Math.round(v.score * 100) }))}
					triage={triageResult ? { urgency: triageResult.urgency, timeframe: triageResult.timeframe } : undefined}
					recommendations={recommendations ? { immediate: recommendations.immediate?.map(a => ({ title: a.title, timeframe: a.timeframe })), followUp: recommendations.followUp?.map(f => ({ title: f.title, timeframe: f.timeframe })) } : undefined}
				/>
			)
			const blob = await createPdf(doc).toBlob()
			const url = URL.createObjectURL(blob)
			const a = document.createElement('a')
			a.href = url
			a.download = 'carelens-results.pdf'
